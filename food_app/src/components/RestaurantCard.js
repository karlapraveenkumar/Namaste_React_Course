...
const RestaurantCard = (props)=>{
    ...
    return (
        <div data-testid="resCard" className="p-4 m-4" >

            <div className="">
                <img className="rounded-lg h-40 w-60" alt="res-logo" 
                    src={
            ...
            <div>
                {name.length > 20 ? <h3 className="font-bold py-4 text-m">{name.substring(0,20)+"..."}</h3>  :<h3 className="font-bold py-4 text-m">{name}</h3>}

                <div className="flex  items-center">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" role="img" aria-hidden="true" 
                        strokecolor="rgba(2, 6, 12, 0.92)" fillcolor="rgba(2, 6, 12, 0.92)"><circle cx="10" cy="10" r="9" 
                        ...
                        <defs><linearGradient id="StoreRating20_svg__paint0_linear_32982_71567" x1="10" y1="1" x2="10" y2="19" gradientUnits="userSpaceOnUse">
                            <stop stopColor="#21973B"></stop><stop offset="1" stopColor="#128540"></stop>
                            </linearGradient>
                        </defs>
                    </svg>
                    <h4 className=" mx-3 font-bold">{avgRating}</h4>
                    <h4 className="font-semibold">{sla.deliveryTime} minutes</h4>
                </div>

                {cuisines.join(", ").length > 20  ? <h4>{cuisines.join(", ").substring(0,20)+"..."}</h4> : <h4>{cuisines.join(", ")}</h4>}
...
export const withPromotedLabel = (RestaurantCard)=>{
    return (props)=>{
        return (
            <div>
                <label className="absolute bg-black text-white m-1 p-1 h-10 ml-5 -mt-2 -z-10 rounded-lg ">Promoted</label>
